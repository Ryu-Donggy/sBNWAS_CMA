...
class WorkPlanController(override var pCon: Context) : IController() {

    var todayWork: WorkPlan? = null
    override fun asFragCreate(): PFragment? {
        vModel = InViewModel()
        frag = WorkListFragment.with(this, vModel, pCon.getString(R.string.msg_1_weeks_work_plan))
    ...
    open inner class InViewModel : PViewModel(pCon) {
        override fun getList(): LiveData<List<PBean>> {
            mList.value = DBManager.withDB().withWorkPlan().selectPlanForMonotor()
            return mList
        }
